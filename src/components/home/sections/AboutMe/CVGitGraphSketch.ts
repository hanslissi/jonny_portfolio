    ...
    return (p: p5) => {
        ...
        const drawCVGitGraph = (canvas: p5.Graphics) => {
            ...
            for (let i = 0; i < cvEntriesCopy.length; i++) {
                ...
                if (cvEntriesCopy[i].type === "education") {
                    ...
                    canvas.line(lastEducationXY.x + circleSize * circleMp, lastEducationXY.y, x - circleSize * circleMp, y);
                ...
                canvas.line(
                    ...
                    lastXY.y + (lastXY.y == y ? 0 : lastXY.y > y ? -circleSize : circleSize),
            ...
            canvas.line(lastXY.x + circleSize * circleMp, lastXY.y, canvas.width, lastXY.y);

            if (lastEducationXY !== lastXY) {
                canvas.stroke(colorEducation);
                canvas.line(lastEducationXY.x + circleSize * circleMp, lastEducationXY.y, p.width, lastEducationXY.y);
            }
        ...
        const getDateTextFromCVEntry = (entry: CVEntry): string => {
            if (entry.dateEnd === undefined) {
                return "since " + entry.dateStart.toLocaleString('default', { month: 'short' }) + ", " + entry.dateStart.getFullYear();
            } else if (entry.dateStart !== entry.dateEnd && entry.dateStart.getMonth() !== entry.dateEnd.getMonth()) {
                return entry.dateStart.toLocaleString('default', { month: 'short' }) + " - " + entry.dateEnd.toLocaleString('default', { month: 'short' }) + ", " + entry.dateStart.getFullYear();
            } else {
                return entry.dateStart.toLocaleString('default', { month: 'short' }) + ", " + entry.dateStart.getFullYear();
            }
        }
        ...
        p.draw = () => {
            ...
            for (let i = 0; i < cvEntriesCopy.length; i++) {
                ...
                if (entry.canvasPosition !== undefined && isOnMouse(entry.canvasPosition, circleSize * circleMp)) {
                    p.stroke(entry.type === 'education' ? colorEducation : colorJob);
                    p.noFill();
                    p.strokeWeight(5);
                    p.circle(entry.canvasPosition.x, entry.canvasPosition.y, circleSize * circleMp * 2);

                    const boundsTitle = urbanistBold.textBounds(entry.title, p.mouseX, p.mouseY, fontSize) as Bounds;
                    const boundsDescription = urbanistRegular.textBounds(entry.description, boundsTitle.x, boundsTitle.y + boundsTitle.h * 2 + padding / 8, fontSize) as Bounds;

                    const rectWidth = boundsDescription.w + padding / 2;
                    const rectHeight = boundsTitle.h + boundsDescription.h + padding / 8 + padding / 2;
                    const rectX = p.constrain(boundsTitle.x - padding / 4, padding / 2, p.width - rectWidth - padding / 2);
                    ...
                    p.text(entry.description, rectX + padding / 4, boundsDescription.y + padding / 4 - rectHeight);

                    p.fill(255, 95);
                    p.text(getDateTextFromCVEntry(entry), rectX + padding / 4 + boundsTitle.w + padding / 4, rectY + padding / 2);
                }
            }
        }
    }
